...
export const MESSAGES = {
  ...
  // eventLabels/concertLabels 是各自類型的顯示字串（例如「大團練（9/25）」，不用重複帶
  // 「活動」兩個字，類型已經由區塊標題表達），分兩個區塊列出比同一份清單混著「活動・」
  // 「音樂會・」前綴好讀——dialog.js 這個確認彈窗（msgEl 是 textContent，不支援 HTML/上色，
  // 卡片寬度只有 300px）沒辦法用顏色區分類型，用區塊分開是純文字能做到的最佳讀法。
  // 每個區塊超過 6 項時只列前 6 個，其餘折成「及其他 N 項」，避免彈窗被塞爆撐出畫面
  applyRosterTemplateBatch: function (name, eventLabels, concertLabels, added, removed) {
    var MAX_PER_SECTION = 6;
    function section(title, labels) {
      if (!labels.length) return '';
      var shown = labels.slice(0, MAX_PER_SECTION);
      var extra = labels.length - shown.length;
      var lines = shown.map(function (label) { return '・' + label; }).join('\n');
      if (extra > 0) lines += '\n（及其他 ' + extra + ' 項）';
      return title + '\n' + lines;
    }
    var total = eventLabels.length + concertLabels.length;
    var sections = [section('活動', eventLabels), section('音樂會', concertLabels)]
      .filter(function (s) { return s; }).join('\n\n');
    var msg = '即將把「' + name + '」套用到以下 ' + total + ' 項，整份覆蓋目前的名單：\n\n' + sections;
    msg += (added || removed) ? ('\n\n總共新增 ' + added + ' 人次、移除 ' + removed + ' 人次') : '\n\n（人員都跟目前名單相同）';
    msg += '。確定要套用嗎？';
    return msg;
  },
