...
- **批次上傳／一份大檔案自動分割成各分部**——這輪 `repertoire-admin.html` 只做「一個分部一次選一個檔案」的個別上傳入口。使用者確認過：一次選多檔逐一對應分部、或上傳一份合訂 PDF 依頁碼自動切成各分部檔案，都留到之後處理「總譜」時一併評估（自動分割需要引入新的 PDF 處理套件、還要做頁碼範圍設定介面，複雜度明顯更高，不適合跟這輪個別上傳的最小實作綁在一起）
...
3. 一般團員：在**目前這張曲目卡片所屬的那場音樂會**的分部指派（`concerts/{concertId}/pieces/{pieceId}/sections/{instrumentName}`）中被指派過這個樂器（不論具體是哪個 `partId`）→ 該樂器底下所有分部可見。**只看這場音樂會自己的指派，不跨音樂會合併判斷**——同一首曲子在 A 音樂會被指派長笛、B 音樂會沒指派或指派別的樂器時，「譜夾」裡 B 音樂會底下那張卡片不會因為 A 音樂會的指派紀錄而跟著開放，跟卡片本身「依音樂會分組」的資料結構一致，也不用跨音樂會撈資料
...
**版面已用 Artifact 示意稿跟使用者確認過**（比照專案既有慣例，例如 8/26 三項後台介面提案先用示意稿再落地）：

- `repertoire-admin.html` 唯讀摘要從「一個樂器一行（超過一個分部才補一行小字列標籤）」改成「一個分部一行」，每行後面接樂譜操作按鈕（已上傳：檢視／更換／移除；還沒上傳：上傳樂譜）——這是既有 8/26 精簡呈現方式的必要調整，因為樂譜操作是逐分部的
- 「我的音樂會」第一區每首曲目底下用一排分部篩選 chip（同樂器底下的分部，例如長笛 1st/2nd），**預設選中使用者在這場音樂會、這首曲子實際被指派到的那個 `partId`**，不是隨便選第一個；如果使用者沒有實際指派紀錄（例如分部長只是想看、自己沒被排到這首曲子，或 admin/conductor/`canViewAllSheetMusic` 純檢視全部），才 fallback 選該樂器底下第一個分部。選中的分部才顯示「檢視樂譜」連結
- 「其他曲目」第二區是扁平清單，不需要分部 chip（全分部已對全團開放，直接列出）
